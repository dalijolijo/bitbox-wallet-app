...
func (headers *Headers) powHash(msg []byte) chainhash.Hash {
	...
	case ltc.MainNetParams.Net:
		...
		return hash
	case btx.MainNetParams.Net: //TODO BTX
		const (
			N = 1024
			r = 1
			p = 1
...
func (headers *Headers) canConnect(dbTx DBTxInterface, tip int, header *wire.BlockHeader) error {
	if tip == 0 {
		...
		if headers.net.Net == chaincfg.MainNetParams.Net || headers.net.Net == ltc.MainNetParams.Net || headers.net.Net == btx.MainNetParams.Net {
